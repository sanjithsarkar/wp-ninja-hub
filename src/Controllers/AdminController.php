...
class AdminController
{
    ...
    public function addMenu(): void
    {
        ...
        add_action('admin_enqueue_scripts', function ($pageHook) use ($hook) {
            ...
            if (!empty($_GET['preview']) && $_GET['preview'] === '1') {
                $this->enqueueDashboardAssetsForPreview();
            } else {
    ...
    {
        if (!empty($_GET['preview']) && $_GET['preview'] === '1') {
            include WPNINJA_HUB_PATH . 'src/Views/admin/preview-page.php';
        } else {
            include WPNINJA_HUB_PATH . 'src/Views/admin/shortcode-page.php';
        }
    }

    private function enqueueDashboardAssetsForPreview(): void
    {
    ...
    private function enqueueAdminAssets(): void
    {
        ...
        $inlineData = 'window.wpNinjaHub = ' . wp_json_encode([
            ...
            'directPreviewUrl' => admin_url('admin.php?page=wp-ninja-hub&preview=1'),
        ]) . ';';
